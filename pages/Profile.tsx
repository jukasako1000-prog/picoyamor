      ...
      <div className="flex flex-col md:flex-row gap-8 items-start">
        ...
        <div className="w-full md:w-64 space-y-4 shrink-0">
          <div className="bg-white rounded-3xl p-6 shadow-soft text-center">
            ...
            <h3 className="font-black text-xl text-text-main">{user.name}</h3>
            <p className="text-xs text-text-muted">{user.email}</p>
          </div>
          
          <div className="bg-white rounded-3xl p-2 shadow-soft space-y-1">
            <button 
              onClick={() => setActiveTab('details')}
              className={`w-full text-left px-6 py-3 rounded-2xl font-bold text-sm transition-all flex items-center gap-3 ${activeTab === 'details' ? 'bg-primary text-white shadow-md' : 'text-text-muted hover:bg-background-light'}`}
            >
              <span className="material-symbols-outlined text-sm">edit_note</span> Mis Datos
            </button>
            <button 
          ...
          {activeTab === 'details' ? (
            <div className="p-8 md:p-12 space-y-8 animate-fade-in">
              ...
              <form onSubmit={handleUpdate} className="space-y-6">
                ...
                <button type="submit" className="bg-primary hover:bg-primary-hover text-white font-black px-12 py-4 rounded-2xl shadow-xl shadow-primary/20 transition-all active:scale-95">
                  Guardar Cambios
                </button>
              </form>
            </div>
