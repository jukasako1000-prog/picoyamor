      ...
      <div className="flex flex-col items-center gap-6">
        ...
        <div className="relative w-full overflow-hidden">
          <div
            ...
            className="flex md:justify-center gap-3 overflow-x-auto hide-scrollbar py-4 px-4 scroll-smooth"
          >
            {categories.map((cat) => (
              <button
                key={cat}
                onClick={() => setFilter(cat)}
                className={`whitespace-nowrap px-10 py-4 rounded-full text-[11px] font-black uppercase tracking-widest transition-all duration-300 ${filter === cat
            ...
            {/* Espaciador final para scroll en móvil */}
            <div className="w-8 shrink-0 md:hidden"></div>
          </div>

          {/* Gradientes laterales para indicar scroll en móvil */}
          <div className="absolute left-0 top-0 bottom-0 w-8 pointer-events-none bg-gradient-to-r from-background-light/40 to-transparent md:hidden z-10"></div>
          <div className="absolute right-0 top-0 bottom-0 w-8 pointer-events-none bg-gradient-to-l from-background-light/40 to-transparent md:hidden z-10"></div>
        </div>

        {/* Barra de progreso de scroll (Solo móvil) */}
        <div className="flex justify-center md:hidden h-1 w-20 bg-primary/10 rounded-full overflow-hidden -mt-2">
          <div
